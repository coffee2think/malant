...
<div style="text-align:center;">
	...
	<% if((currentPage - limit) < startPage && (currentPage - limit) > 1){  //이전그룹이 있다면 %>
		<a href="/malant/<%= urlMapping %>?page=<%= startPage - limit %>">[이전그룹]</a> &nbsp;
	<% }else{ //이전그룹이 없다면 %>
		[이전그룹] &nbsp;
	<% } %>
	...
	<% if((currentPage + limit) > endPage && (currentPage + limit) < maxPage){  //다음그룹이 있다면 %>
		<a href="/malant/<%= urlMapping %>?page=<%= startPage + limit %>">[다음그룹]</a> &nbsp;
	<% }else{ //다음그룹이 없다면 %>
		[다음그룹] &nbsp;
	<% } %>
...
<div style="text-align:center;">
	...
	<%-- 이전 페이지 그룹으로 이동 --%>
	<% if((currentPage - limit) < startPage && (currentPage - limit) > 1){  //이전그룹이 있다면 %>
		<a href="/malant<%= urlMapping %>?page=<%= startPage - limit %>&action=<%= action %>&keyword=<%= keyword %>">[이전그룹]</a> &nbsp;
	<% }else{ //이전그룹이 없다면 %>
		[이전그룹] &nbsp;
	<% } %>
	...
	<%-- 다음 페이지 그룹으로 이동 --%>
	<% if((currentPage + limit) > endPage && (currentPage + limit) < maxPage){  //다음그룹이 있다면 %>
		<a href="/malant/<%= urlMapping %>?page=<%= startPage + limit %>&action=<%= action %>&keyword=<%= keyword %>">[다음그룹]</a> &nbsp;
	<% }else{ //다음그룹이 없다면 %>
		[다음그룹] &nbsp;
	<% } %>
...
<div style="text-align:center;">
	...
	<%-- 이전 페이지 그룹으로 이동 --%>
	<% if((currentPage - limit) < startPage && (currentPage - limit) > 1){  //이전그룹이 있다면 %>
		<a href="/malant/<%= urlMapping %>?page=<%= startPage - limit %>&action=<%= action %>&begin=<%= begin %>&end=<%= end %>">[이전그룹]</a> &nbsp;
	<% }else{ //이전그룹이 없다면 %>
		[이전그룹] &nbsp;
	<% } %>
