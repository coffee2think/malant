	 ...
	 $.ajax({
	        ...
	        success: function(data) {
	            ...
	            for (var i in json.blist) {
	                ...
	                values += "</td><td><a href='/malant/bdetail?bno=" + <%= json.blist[i].bno %> + "'>" + decodeURIComponent(json.blist[i].btitle) + "</a></td><td align='center'>" 
...
<body>
		...
		<% for(Board b : list){ %>
		<table onclick="javascript:location.href='/malant/bdetail?bno=<%= b.getBoardNo() %>&page=<%= nowPage %>';" width="500" border="0" style="display: block; margin:10px; border-radius : 10px; border : 1px solid  rgba(154, 179, 213, 0.2); background: rgba(154, 179, 213, 0.1);">
			<tr height="30">
