...
public class DiaryDao {
	...
	public ArrayList<Diary> selectList(Connection conn, String userNo, Paging paging) {
		...
		String query = "select *  "
				+ "from (select rownum rnum, diary_id, user_no,  "
	...
	public int insertNewDiray(Connection conn, Diary diary) {
		...
		String query = "insert into my_diary  "
				+ "(user_no, diary_id, diary_writing_date, diary_content )  "
...
//			System.out.println(result);
	...
	public Diary selectDiary(Connection conn, String diaryId) {
		Diary diary = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		
		String query = "select * from my_diary "
				+ "where diary_id = ?";
	...
	public ArrayList<MyDiaryPhotoes> selectDiaryPhotoes(Connection conn, int diaryId) {
		...
		String query = "select * "
					+ "from my_diary_photoes where diary_id = ?";
	...
	public int insertMyDiaryPhoto(Connection conn, int diaryId, String fname) {
		...
		String query = "insert into my_diary_photoes  "
				+ "(photo_id, diary_id, file_name)  "
	...
	/*
	 * public int selectPhotoLastSeq(Connection conn) { int seqId = 0;
	 * PreparedStatement pstmt = null; ResultSet rset = null;
	 * 
	 * String query1 = "select count(*) from my_diary_photoes"; String query2 =
	 * "select MAX(photo_id) from my_diary_photoes"; String query3 =
	 * "select MAX(photo_id) from my_diary_photoes";
	 * 
	 * try { pstmt = conn.prepareStatement(query1); rset = pstmt.executeQuery(); int
	 * count = 0; if(rset.next()) { count = rset.getInt(1); }
	 * 
	 * pstmt = conn.prepareStatement(query2); rset = pstmt.executeQuery();
	 * 
	 * if(rset.next()) { seqId = rset.getInt(1); }
	 * 
