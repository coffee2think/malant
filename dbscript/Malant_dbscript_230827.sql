...
DROP TABLE "ST_BANNER" CASCADE CONSTRAINTS;
...
CREATE TABLE member (
	user_no VARCHAR2(10) NOT NULL, /* 회원고유식별번호 */
	user_id VARCHAR2(12) NOT NULL, /* 아이디 */
	email VARCHAR2(255) NOT NULL, /* 이메일 */
	phone VARCHAR2(13), /* 휴대폰 번호 */
	user_pwd VARCHAR2(64), /* 비밀번호 */
	password_update_date DATE DEFAULT SYSDATE, /* 비밀번호 업데이트 날짜 */
	nickname VARCHAR2(24) NOT NULL, /* 닉네임 */
	profile_img VARCHAR2(255), /* 프로필사진url */
	sign_type VARCHAR2(15) DEFAULT 'DIRECT' NOT NULL, /* 로그인 유형 */
	name VARCHAR2(15), /* 이름 */
	address VARCHAR2(255), /* 주소 */
	reception_notification CHAR(1) DEFAULT 'Y' NOT NULL, /* 식물 관리 알림 이메일 수신 여부 */
	reception_ad CHAR(1) DEFAULT 'Y' NOT NULL, /* 광고 관련 이메일 수신 여부 */
	access_permission CHAR(1) DEFAULT 'N' NOT NULL, /* 로그인 제한 여부 */
...
CREATE TABLE notice (
	...
	content_image VARCHAR2(255), /* 공지글 이미지 */
...
CREATE TABLE st_product (
	...
	product_description VARCHAR2(3000) NOT NULL, /* 상품 설명 */
...
CREATE TABLE st_banner (
	banner_no VARCHAR2(10) NOT NULL, /* 배너 번호 */
	admin_no VARCHAR2(10) NOT NULL, /* 관리자 번호 */
	priority NUMBER DEFAULT 1 NOT NULL, /* 우선순위 */
	banner_title VARCHAR2(90) NOT NULL, /* 공지 제목 */
	banner_image VARCHAR2(255) NOT NULL, /* 배너 이미지 */
	link_url VARCHAR2(255), /* 연결될 주소 */
	banner_start DATE, /* 배너 시작일 */
	banner_end DATE, /* 배너 종료일 */
	post_date DATE DEFAULT SYSDATE NOT NULL, /* 배너 등록일 */
	viewcount NUMBER DEFAULT 0 NOT NULL /* 조회수 */
);

ALTER TABLE st_banner
	ADD
		CONSTRAINT PK_st_banner
		PRIMARY KEY (
			banner_no
		);
...
ALTER TABLE st_banner
	ADD
		CONSTRAINT FK_admin_TO_st_banner
		FOREIGN KEY (
			admin_no
		)
		REFERENCES admin (
